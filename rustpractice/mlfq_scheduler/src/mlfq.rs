...
impl MLFQ {
    ...
    pub fn add_process(&mut self, mut process: Process) {
        ...
        // Ensure the process's priority is within range, if out of range, it will be set to the nearest valid
        // Clamp the priority to ensure it's within the valid range
        process.priority = process.priority.clamp(0, self.num_levels - 1);  //clamp ensures priority is within valid range, sets value to minimum if it's too low, max if too high, or leave if already in range

        // Add the process to the appropriate queue, which could have been adjusted
    ...
    pub fn execute_process(&mut self, queue_index: usize) {
        ...
        // Check if queue_index is valid
        if queue_index >= self.num_levels{
            return; //queue has invalid index, therefore exit since queue indeax is out of bounds
        }

        //try to get a process from the specified queue
        if let Some(mut process) = self.queues[queue_index].pop() {
            ...
            // Update process and scheduler times
            ...
            if process.remaining_time > 0 {
                //process is not completed, move it to a lower priority queue or keep it in the lowest
                // Move to a lower priority queue if it used its full time quantum
                let next_priority = (process.priority + 1).min(self.num_levels - 1);
                process.priority = next_priority;
                self.queues[next_priority].push(process);
            }
            //if process is completed, remaining time of 0, it will not be put back in the queue
    ...
    pub fn priority_boost(&mut self) {
     ...
     let mut boosted_processes = Vec::new();
        ...
        // Collect all processes from lower priority queues, except the highest ones
        for queue in self.queues.iter_mut().skip(1) {
            //drain moves all elements from the queue to boosted_processes
            boosted_processes.extend(queue.drain(..));
        }

        // Reset their priorities and add them to the highest priority queue
        for process in &mut boosted_processes {
            process.priority = 0;
        }
